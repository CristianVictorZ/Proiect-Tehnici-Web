...
app.get("/produse", function(req, res){
    ...
    if(req.query.tip)
    {
        conditieQuerry = ` where tip_produs = '${req.query.tip}'`;
    }
    console.log(`select * from prajituri ${conditieQuerry}`);
    client.query("select * from unnest(enum_range(null::categ_prajitura))", function(err, rezOptiuni){
        client.query(`select * from prajituri ${conditieQuerry}`, function(err, rez){        
            if(err){            
                console.log(err);            
                afisareEroare(res, 2);        
            }        
