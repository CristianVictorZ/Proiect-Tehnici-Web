...
var client= new Client({database:"cti_2024",
        user:"cristian",
        password:"TWCriviza2003",
        host:"localhost",
        port:5432});
client.connect();

client.query("select * from unnest(enum_range(null::dimensiuni_pizza))", function(err, rez){
    console.log(rez);
});
...
client.query("select * from unnest(enum_range(null::tipuri_blat))", function(err, rezCategorie){
    if (err){
        console.log(err);
    }
    else{
...
app.use(function(req, res, next){
    client.query("select * from unnest(enum_range(null::dimensiuni_pizza))", function(err, rezOptiuni){
...
app.get("/produse", function(req, res){
    ...
    if(req.query.tip)
    {
        conditieQuerry = ` where tip_blat = '${req.query.tip}'`;
    }
    console.log(`select * from pizza ${conditieQuerry}`);
    client.query("select * from unnest(enum_range(null::dimensiuni_pizza))", function(err, rezOptiuni){
        client.query(`select * from pizza ${conditieQuerry}`, function(err, rez){        
            if(err){            
                console.log(err);            
                afisareEroare(res, 2);        
            }        
...
app.get("/produs/:id", function(req, res){    
    client.query(`select * from pizza where id=${req.params.id}`, function(err, rez){        
        if (err){            
            console.log(err);            
            afisareEroare(res, 2);        
        }        
...
app.post("/produse_cos",function(req, res){
    ...
    if(req.body.ids_prod.length!=0){
        ...
        AccesBD.getInstanta().select({tabel:"pizza", campuri:"nume,descriere,pret,gramaj,imagine".split(","),conditiiAnd:[`id in (${req.body.ids_prod})`]},
        function(err, rez){
            if(err)
                res.send([]);
            else
...
client.query("select id from pizza", function(err, rez){
...
app.get("/useri", function(req, res){
    ...
    */
    if(req?.utilizator?.areDrepturi(Drepturi.vizualizareUtilizatori)){
        var obiectComanda= {
            tabel:"utilizatori",
            campuri:["*"],
            conditiiAnd:[],
...
app.post("/sterge_utiliz", function(req, res){
    ...
    */
    ...
    if(req?.utilizator?.areDrepturi(Drepturi.stergereUtilizatori)){
        var formular= new formidable.IncomingForm();
